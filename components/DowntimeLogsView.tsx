...
import { AlertOctagon, Clock, Wrench, AlertTriangle, FileText, Search, Filter } from 'lucide-react';

interface DowntimeLogsViewProps {
  logs: DowntimeLog[];
}

export const DowntimeLogsView: React.FC<DowntimeLogsViewProps> = ({ logs }) => {
                ...
                filteredLogs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 text-sm text-slate-600">
                      <div>{new Date(log.date).toLocaleDateString('th-TH')}</div>
